...
import { LuCalendar, LuUsers, LuX } from "react-icons/lu";
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-50 bg-black w-65 bg-linear-to-b from-[#0F172B] via-[#0F172B] to-[#020618] border-r border-white/4 text-white lex flex-col transition-all duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex items-center gap-3 py-6 px-5 border-b border-white/6">
          ...
          <div className="flex justify-between items-center flex-1">
            ...
            <button onClick={() => setIsOpen(false)} className="cursor-pointer"> <LuX size={20} /> </button>
          </div>
        </div>
      ...
      <div className="sidebar w-65 bg-linear-to-b from-[#0F172B] via-[#0F172B] to-[#020618] border-r border-white/4 text-white hidden md:flex flex-col">
      ...
      <button onClick={() => setIsOpen(prev => !prev)} className="absolute top-4 left-4 cursor-pointer md:hidden">
        {!isOpen && <MdMenu size={30} />}
      </button>
